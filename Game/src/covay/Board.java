...
public class Board extends JPanel {
	...
	// Độ dung sai cho phép click gần giao điểm (đơn vị: pixel)
			...
			public void mousePressed(MouseEvent e) {
				...
				for (int i = 0; i < size; i++) {
					for (int j = 0; j < size; j++) {
						...
						// Cho phép click gần giao điểm, trong phạm vi dung sai
						if (Math.abs(x - eX) <= tolerance && Math.abs(y - eY) <= tolerance) {
							if (cell.getValue().equals(Cell.EMTY_VALUE)) {
								cell.setValue(currentPlayer);
								currentPlayer = currentPlayer.equals(Cell.BLACK_VALUE) ? Cell.WHITE_VALUE
										: Cell.BLACK_VALUE;
